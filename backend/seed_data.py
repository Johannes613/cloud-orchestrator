...
from models import ApplicationCreate, CPUResources, MemoryResources, StorageResources, Resources
from datetime import datetime

async def seed_applications():
    """Seed Firebase with initial application data"""
    ...
    print("Seeding applications to Firebase...")
    
    for app_data in applications:
        try:
            app_create = ApplicationCreate(**app_data)
            application = await service.create_application(app_create)
            if application:
                print(f"✓ Created application: {application.name}")
            else:
                print(f"✗ Failed to create application: {app_data['name']}")
